...
ggplot(pileup_parse) + geom_point(aes(depth, prop, color = max_copy, shape = platform)) + 
  scale_x_log10() + facet_wrap(~exp, nrow = 1) +
  labs (x = "log(Depth)", y = "Observed Proportions", shape = "Sequencing\nPlatform", color = "Copy Ratio")

pileup_parse$max_copy <- as.factor(pileup_parse$max_copy)
pileup_parse <- cbind(pileup_parse, colsplit(pileup_parse$exp, ":", names = c("high", "low")))
ggplot(pileup_parse) + 
  geom_hline(aes(yintercept = 100), linetype = 2) +
  geom_hline(aes(yintercept = 500), linetype = 2) +
  geom_boxplot(data= pileup_parse[pileup_parse$max_copy == pileup_parse$high,], 
                                    aes(max_copy, depth, color = max_copy)) +
  geom_boxplot(data= pileup_parse[pileup_parse$max_copy != pileup_parse$high,], 
              aes(max_copy, depth)) +
  geom_hline(aes(yintercept = 100, label = 100), linetype = 2) +
  facet_wrap(~exp, nrow = 1) + scale_y_log10() + 
  labs (y = "log(Depth)", x = "Copy Ratio", shape = "Sequencing\nPlatform", color = "Copy Ratio")
